...
impl MinerManager {
    ...
    pub fn launch_cpu_miner(
        ...
        #[cold]
        fn found_block(send_channel: &Sender<KaspadRequest>, block: RpcBlock) -> Result<(), Error> {
            let block_hash = block.block_hash().expect("We just got it from the state, we should be able to hash it");
            send_channel.blocking_send(KaspadRequest::submit_block(block))?;
